...
const Contact: React.FC<ContactProps> = ({ selectedService }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      // Basic validation
      if (formState.name && formState.email.includes('@')) {
        setStatus('success');
        setFormState({ name: '', email: '', service: 'web-dev', message: '' });
      } else {
        setStatus('error');
      }
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-display font-bold text-white mb-6">
            Let's Build <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-rose-500">Something Great</span>
          </h2>
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <optgroup label="Business Bundles">
                  <option value="startup">Kickstarter Pack</option>
                  <option value="growth">Growth Accelerator</option>
                  <option value="enterprise">Market Dominator</option>
                </optgroup>
              </select>
            </div>
